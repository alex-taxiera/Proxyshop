...
def apply_fx_gradient_overlay(action: ActionDescriptor, fx: EffectGradientOverlay) -> None:
    ...
    d1.putEnumerated(sID("mode"), sID("blendMode"), sID(fx.blend_mode))
    d1.putUnitDouble(sID("opacity"), sID("percentUnit"),  fx.opacity)
    ...
    d1.putBoolean(sID("dither"), fx.dither)
    d1.putEnumerated(cID("gs99"), sID("gradientInterpolationMethodType"), sID(fx.method))
